...
const Toast: React.FC<ToastProps> = ({ message, type, onClose }) => {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-[9999] animate-slide-in-up w-[90%] md:w-auto">
      <div className={`flex items-center gap-3 px-5 py-3 rounded-lg shadow-2xl border backdrop-blur-md ${
        type === 'success' 
                 ...
                 return (
                  <button 
                    ...
                    className={`w-full text-left p-4 rounded-lg border transition-all duration-200 group relative overflow-hidden ${
                      isSelected ? 'bg-slate-750 border-l-4 border-l-blue-500 border-y-slate-700 border-r-slate-700 shadow-md ring-1 ring-black/20' : 'bg-slate-800 border-transparent hover:bg-slate-700/50 hover:translate-x-1'
                    }`}
                  >
                    ...
                    <div className="flex justify-between items-start mb-1 relative z-10">
                      ...
                      <span className={`text-[10px] uppercase font-bold px-2 py-0.5 rounded border shadow-sm ${
                        v.status === VehicleStatus.MOVING ? 'bg-blue-500/10 text-blue-400 border-blue-500/20' :
